...
from redisvl.storage import BaseStorage, JsonStorage, HashStorage
...
def test_key_formatting(storage_instance):
    ...
    assert generated_key == f"test:{key}", "The generated key does not match the expected format."


def test_create_key(storage_instance):
    key_field = 'id'
    obj = {key_field: '1234'}
    expected_key = f"{storage_instance._prefix}{storage_instance._key_separator}{obj[key_field]}"
    generated_key = storage_instance._create_key(obj, key_field)
    assert generated_key == expected_key, "The generated key does not match the expected format."
