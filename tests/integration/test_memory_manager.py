...
@pytest.fixture
def interaction():
    return Interaction(session_id="test", prompt="foo", response="bar")
...
def test_memory_params(memory):
    ...
    assert memory.semantic_threshold == 0.5
...
def test_seek_memory(memory, interaction):
    ...
    assert memory.seek_range("test", 0, 4) == [interaction for _ in range(5)]
...
def test_seek_relevant(memory, interaction):
    ...
    results = memory.seek_relevant("test", context=context, n=3)
