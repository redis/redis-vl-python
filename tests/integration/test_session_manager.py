...
def test_include_preamble():
    ...
    session = StandardSessionManager(name="test_app", session_id="123", user_id="abc", preamble=preamble)
...
def test_specify_redis_client(client):
    session = StandardSessionManager(name="test_app", session_id="abc", user_id="123", redis_client=client)
...
def test_standard_store_and_fetch(standard_session):
    ...
    standard_session.store(prompt="first prompt",
                           response="first response")
    standard_session.store(prompt="second prompt",
                            response="second response")
    standard_session.store(prompt="third prompt",
                            response="third response")
    standard_session.store(prompt="fourth prompt",
                            response="fourth response")
    standard_session.store(prompt="fifth prompt",
    ...
    assert partial_context == [{"role": "_preamble", "_content": ""},
    ...
    assert full_context == [{"role": "_preamble", "_content": ""},
...
def test_standard_set_scope(standard_session, app_name, user_id, session_id):
    ...
    assert previous_context == [{"role":"_preamble", "_content":""},
...
def test_standard_fetch_raw(standard_session):
    ...
    assert len(raw) == 2
    assert raw[0].keys() == {"id_field", "prompt", "response", "timestamp", "token_count"}
...
def test_semantic_include_preamble():
    ...
    preamble = "system level instruction to llm."
    session = SemanticSessionManager(name="test_app", session_id="123", user_id="abc", preamble=preamble)
    assert session._preamble == {"role": "_preamble", "_content": preamble}
...
def test_semantic_specify_client(client):
    session = SemanticSessionManager(name="test_app", session_id="abc", user_id="123", redis_client=client)
    assert isinstance(session._client, type(client))
...
def test_semantic_store_and_fetch_recent(semantic_session):
    ...
    semantic_session.store(prompt="first prompt",
                           response="first response")
    semantic_session.store(prompt="second prompt",
                            response="second response")
    semantic_session.store(prompt="third prompt",
                            response="third response")
    semantic_session.store(prompt="fourth prompt",
                            response="fourth response")
    semantic_session.store(prompt="fifth prompt",
                            response="fifth response")
...
def test_semantic_store_and_fetch_relevant(semantic_session):
    semantic_session.store(prompt="list of common fruits",
                           response="apples, oranges, bananas, strawberries")
    semantic_session.store(prompt="list of common vegetables",
                            response="carrots, broccoli, onions, spinach")
    semantic_session.store(prompt="winter sports in the olympics",
                            response="downhill skiing, ice skating, luge")

    # test default distance metric
    default_context = semantic_session.fetch_relevant('set of common fruits like apples and bananas')
    assert len(default_context) == 3 
    assert default_context[1] == {"role":"_user", "_content":"list of common fruits"}
    assert default_context[2] == {"role":"_llm", "_content":"apples, oranges, bananas, strawberries"}
