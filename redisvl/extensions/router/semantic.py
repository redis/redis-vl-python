...
class SemanticRouter(BaseModel):
    ...
    def _apply_cost_optimization(self, matches: List[RouteMatch]) -> List[RouteMatch]:
        ...
        for match in matches:
            try:
                cost = float(match.metadata.get("cost_per_1k_input", 0))
            except (TypeError, ValueError):
                cost = 0.0
            # Add cost penalty to distance (normalized)
            adjusted_distance = (match.distance or 0.0) + (cost * cost_weight)
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        for pr in config.routes:
            ...
            # Use pre-computed vectors (convert to buffer using vectorizer dtype)
            for ref in pr.references:
                vector_buffer = array_to_buffer(ref.vector, vectorizer.dtype)
