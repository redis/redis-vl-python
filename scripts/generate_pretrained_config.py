#!/usr/bin/env python3
"""Generate pretrained router configurations with pre-computed embeddings.
...
def main():
    ...
    for route_def in ROUTES:
        print(
            f"Embedding {len(route_def['references'])} references for route '{route_def['name']}'..."
        )
        vectors = vectorizer.embed_many(route_def["references"])

        references = [
            PretrainedReference(text=text, vector=vec)
            for text, vec in zip(route_def["references"], vectors)
        ]
        ...
        pretrained_routes.append(
            PretrainedRoute(
                ...
                metadata=route_def["metadata"],
                distance_threshold=route_def["distance_threshold"],
            )
        )
    ...
    print(f"Total routes: {len(pretrained_routes)}")
    for route in pretrained_routes:
        print(f"  {route.name}: {len(route.references)} references, model={route.model}")
