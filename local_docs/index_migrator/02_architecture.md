...
> **Status**: Phase 1 is implemented and shipped. This document describes the original architecture design. See [04_implementation_summary.md](./04_implementation_summary.md) for the actual module map.
...
The implementation adds migration capabilities on top of existing RedisVL primitives instead of creating a separate control plane.
...
sequenceDiagram
    ...
    Op->>CLI: migrate apply --plan migration_plan.yaml
    CLI->>DB: quantize vectors (if needed)
    CLI->>DB: rename fields/keys (if needed)
